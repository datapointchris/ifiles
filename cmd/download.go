...
func init() {
	downloadCmd.Flags().StringVar(&getArchive, "archive", "", "archive format for a directory: zip or tar.gz")
	mustCompleteFlag(downloadCmd, "archive", cobra.FixedCompletions(
		[]string{filebrowser.ArchiveZip, filebrowser.ArchiveTarGz},
		cobra.ShellCompDirectiveNoFileComp,
	))
	downloadCmd.Flags().BoolVar(&getOverride, "override", false, "replace an existing local file")
	downloadCmd.Flags().BoolVar(&getNoResume, "no-resume", false, "ignore a partial download and start over")
